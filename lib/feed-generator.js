                        ...
                        if (typeof paragraph.richtext === 'string') {
                            ...
                            if (paragraph.richtext.startsWith('<h3>')) {
                                ...
                                if (firstParagraph) {
                                    ...
                                    if (electionStory) {
                                        if (self.electionModuleImages && self.electionModuleImages[0]) {
                                            let electionModuleImage = self.electionModuleImages[0].url,
                                            ...
                                            // feed += `<hr class="style-id:headerDivider"><h3 class="style-id:presResultsHeader">${imgTitle}</h3>`;
                                            feed += `<hr class="style-id:headerDivider">${imgTitle}`;
                                            feed += `<img src="${electionModuleImage}"/>`;
                                            feed += `<hr class="style-id:relatedContentDivider"><p class="style-id:caption"><span class="style-id:caption">${imgCaption}</span></p>`;
                                            feed += `${electionModuleLink1}`;

                                            if (!electionModuleLink2 === '') {
                                                feed += `${electionModuleLink2}`;
                                            }
                                ...
                                if (paragraph.elements[0].target.type === 'video') {
                                    contentModel.docs[0].relatedMedia.media.some((item) => {
                                        if (item.location === paragraph.id && item.cuts) {
                                            ...
                                            feed += `<video src="${item.cdnUrls['1280x720_3500k_mp4']}" poster="${item.cuts[imageCut].url.replace(/i2.cdn.turner.com\/cnnnext\/dam\/assets\//, 'dynaimage.cdn.turner.com/gns/gns/e_trim/').replace(/\-super\-169/, '').replace(/-live-video/, '')}"></video>`;

...
class FeedGenerator {
     ...
     */
    get urls() {
        const debugLog = debug('cnn-google-newsstand:FeedGenerator:urls-getter');

        debugLog(`Getting this._urls: ${this._urls}`);
    ...
    generateFeed(contentModels) {
        ...
        return new Promise((resolve) => {
            ...
            contentModels.forEach((contentModel) => {
                ...
                feed += '<item>';
                feed += `<title>${contentModel.docs[0].headline.trim()}</title>`;

...
class FeedGenerator {
            ...
            contentModels.forEach((contentModel) => {
                ...
                function isValidStory(doc) {
                    let cmsStoryType = doc.storyType || doc.type || '',
                        allowList = ['story', 'special article', 'post', 'gallery', 'article'];
                ...
                if (contentModel.docs[0].type === 'article') {
                                ...
                                if (config.get('gnsEnableInlineGalleries') === true || config.get('gnsEnableInlineGalleries') === 'true') {
                                    ...
                                    if (item.referenceType === 'gallery') {
                                        ...
                                        if (articleTease && articleTease.cuts) {
                                            ...
                                            feed += `<media:content url="${articleTease.cuts[imageCut].url.replace(/i2.cdn.turner.com\/cnnnext\/dam\/assets\//, 'dynaimage.cdn.turner.com/gns/gns/e_trim/').replace(/\-super\-169/, '').replace(/-live-video/, '')}" type="image/jpg" expression="full">`;
                                            feed += `<media:thumbnail url="${articleTease.cuts[imageCut].url.replace(/i2.cdn.turner.com\/cnnnext\/dam\/assets\//, 'dynaimage.cdn.turner.com/gns/gns/e_trim/').replace(/\-super\-169/, '').replace(/-live-video/, '')}"/>`;
                                            feed += '</media:content>';
                                            firstItemInPageTop = true;
                                        } else if (config.get('gnsGenericThumbImage')) {
                                            item.cuts = {
                                        ...
                                        if (item.slides) {
                                            feed += '<section class="type:slideshow">';
                                            item.slides.forEach(function (slide) {
                                                ...
                                                if (slide && slide.cuts) {
                                                    ...
                                                    feed += '<figure>';
                                                    feed += `<img src="${slide.cuts[galleryImageCut].url.replace(/i2.cdn.turner.com\/cnnnext\/dam\/assets\//, 'dynaimage.cdn.turner.com/gns/gns/e_trim/').replace(/\-super\-169/, '').replace(/-live-video/, '')}"/>`;
                                                    ...
                                                    if (slide.caption) {
                                                        ...
                                                        if (galleryImageCutCaption) {
                                                            feed += `<p class="style-id:caption"><q class="style-id:caption">${galleryImageCutCaption}</q></p>`;
                                                        }

                                                        if (slide.credit) {
                                                            feed += `<p class="style-id:gallery-credit"><q class="style-id:gallery-credit">${slide.credit}</q></p>`;
                                                        }
                                                        feed += '</figcaption>';
                                                    }
                    ...
                    templateType = containsVideo ? '&c32=adbp%3Acontent&v32=adbp%3Acontent':'&c32=adbp%3Acontent&v32=adbp%3Acontent';
    ...
    ${moment(contentModel.docs[0].lastPublishDate).tz('America/New_York').format('h:mm A z MMMM D, YYYY')}"/>`;
                    ...
                    if (contentModel.docs[0].slides  && contentModel.docs[0].slides.length) {
                        ...
                        contentModel.docs[0].slides.forEach((slide, index) => {
                            ...
                            if (index === 0) {
                                ...
                                feed += `<media:thumbnail url="${slide.image.url}"/>`;
                ...
                ${moment(contentModel.docs[0].lastPublishDate).tz('America/New_York').format('h:mm A z MMMM D, YYYY')}"/>`;
                                ...
                                feed += '<div class="style-id:cnn-gallery">';
                            }
                            ...
                            if (slide.cuts) {
                                ...
                                feed += `<img src="${slide.cuts[imageCut].url.replace(/i2.cdn.turner.com\/cnnnext\/dam\/assets\//, 'dynaimage.cdn.turner.com/gns/gns/e_trim/').replace(/\-super\-169/, '').replace(/-live-video/, '')}"/>`;
                            }
                            ...
                            if (caption) {
                                feed += `<hr class="style-id:relatedContentDivider"><p class="style-id:caption"><q class="style-id:caption">${caption}</q></p>`;
                            }

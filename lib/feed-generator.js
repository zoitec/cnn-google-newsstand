...
class FeedGenerator {
    ...
    ${moment(contentModel.docs[0].lastPublishDate).tz('America/New_York').format('h:mm A z MMMM D, YYYY')}"/>`;
                ...
                ${moment(contentModel.docs[0].lastPublishDate).tz('America/New_York').format('h:mm A z MMMM D, YYYY')}"/>`;
                            ...
                            if (caption) {
                                feed += `<hr class="style-id:relatedContentDivider"><p class="style-id:gallery-caption"><q class="style-id:gallery-caption">${caption}</q></p>`;
                                //feed += `<hr class="style-id:relatedContentDivider"><figcaption>${caption}`;
                                /*if (slide.credit) {
                                    feed += `<span class="copyright">${slide.credit}</span>`;
                                }
                                feed += `</figcaption>`;*/
                            }

                            if (slide.credit) {
                                //feed += `<p class="style-id:gallery-credit"><span class="style-id:gallery-credit">${slide.credit}</span></p>`;

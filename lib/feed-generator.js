        ...
        return new Promise((resolve) => {
            ...
            contentModels.forEach((contentModel) => {
                ...
                if (contentModel.docs[0].type === 'article') {
                        ...
                        if (item.location === 'pageTop') {
                            ...
                            if (item.type === 'reference') {
                                if (item.referenceType === 'video') {
                                    imageCut = (item.cuts.exlarge16to9) ? 'exlarge16to9' : 'large16to9'; // sometimes exlarge16to9 doesn't exist, fall back to large16to9
                                    feed += '<media:content url="http://ht.cdn.turner.com/cnn/big/sports/2016/08/07/business-of-oympics-cnni-mann-intv.cnn_ios_5500.mp4" medium="video">';
                                    feed += '<media:title>Brand names battle for gold at Olympic Games 1</media:title>';
                                    feed += '<media:description><![CDATA[Brand names battle for gold at Olympic Games 2]]></media:description>';
                                    feed += '<media:credit role="author" scheme="urn:ebu"><![CDATA[CNN]]></media:credit>';
                    ...
                    feed += `<meta itemprop="headline" content="${contentModel.docs[0].headline.trim()}"/>`; // TODO escape quotes - maybe
                    feed += '<meta itemprop="alternativeHeadline" content="This is the alternative headline"/>';
                    feed += '<meta itemprop="genre" content="This is the genre"/>';
                    ...
                    contentModel.docs[0].body.paragraphs.forEach(function (paragraph) {
                        ...
                        // TODO - only grabbing the first item in the elements array
                        if (paragraph.elements.length > 0) {
                            if (paragraph.elements[0].type === 'handle' && paragraph.elements[0].target.type === 'image') {
                                contentModel.docs[0].relatedMedia.media.some((item) => {
                                    if (item.location === paragraph.id) {
                                        let caption = undefined;

                                        feed += '<figure>';
                                        feed += `<img src="${item.cuts.exlarge16to9.url}" height="${item.cuts.exlarge16to9.height}" width="${item.cuts.exlarge16to9.width}"/>`;
                                        ...
                                        if (!item.caption && item.photographer) {
                                            caption = item.photographer;
                                        }

                                        if (item.caption && item.photographer) {
                                            caption = `${item.caption} - ${item.photographer}`;
                                        }
                ...
                if (contentModel.docs[0].type === 'video') {
                    ...
                    contentModel.docs[0].relatedMedia.media.some((item) => {
                        if (item.type === 'image' && item.location === 'thumbnails') {
                            feed += `<media:thumbnail url="${item.cuts.exlarge16to9.url}" height="${item.cuts.exlarge16to9.height}" width="${item.cuts.exlarge16to9.width}" />`;
                            return true;
                        }
                    });

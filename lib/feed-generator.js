                ...
                if (contentModel.docs[0].type === 'article') {
                    ...
                    if (contentModel.docs[0].body.notes && contentModel.docs[0].body.notes.www) {
                        if (typeof contentModel.docs[0].body.notes.www === 'string') {
                            feed += `<p class="editorsNote">${contentModel.docs[0].body.notes.www}</p>`;
                        }
                    }

                    // handle paragraphs and their related elements
                                ...
                                if (paragraph.elements[0].target.type === 'image') {
                                    contentModel.docs[0].relatedMedia.media.some((item) => {
                                        if (item.location === paragraph.id) {
                                            ...
                                            if (!item.caption && item.photographer) {
                                                caption = `<span class="copyright">${item.photographer}</span>`;
                                            }

                                            if (item.caption && item.photographer) {
                                                caption = `${item.caption} <span class="copyright">${item.photographer}</span>`;
                                            }
                    ...
                    feed += '<div class="footer"><span class="copyright">&copy;2016 Cable News Network, Inc. A Time Warner Company. All Rights Reserved.</span></div>';

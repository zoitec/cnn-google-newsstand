                ...
                if (contentModel.docs[0].type === 'article') {
                    ...
                    contentModel.docs[0].relatedMedia.media.forEach((item) => {
                        ...
                        if (item.location === 'pageTop') {
                            ...
                            if (item.type === 'image') {
                                imageCut = (item.cuts || item.cuts.exlarge16to9) ? 'exlarge16to9' : 'large16to9'; // sometimes exlarge16to9 doesn't exist, fall back to large16to9
                ...
                if (contentModel.docs[0].type === 'video') {
                    ...
                    contentModel.docs[0].relatedMedia.media.some((item) => {
                        if (item.type === 'image' && item.location === 'thumbnails') {
                            let imageCut = (item.cuts || item.cuts.exlarge16to9) ? 'exlarge16to9' : 'large16to9'; // sometimes exlarge16to9 doesn't exist, fall back to large16to9
                    ...
                    feed += '<content:encoded><![CDATA[';
                    feed += `<p class="style-id:mediaFirstParagraph"><span class="style-id:mediaFirstParagraphText">${contentModel.docs[0].description[0].richtext}</span></p>`;
                    feed += ']]></content:encoded>';
                }

...
class FeedGenerator {
    ...
    ${moment(contentModel.docs[0].lastPublishDate).tz('America/New_York').format('h:mm A z MMMM D, YYYY')}"/>`;
                ...
                ${moment(contentModel.docs[0].lastPublishDate).tz('America/New_York').format('h:mm A z MMMM D, YYYY')}"/>`;
                        ...
                        feed += '</div>';
                        feed += `<p class="style-id:footer"><span class="style-id:mediaCopyright">&copy;${new Date().getFullYear()} Cable News Network, Inc. A Time Warner Company. All Rights Reserved.</span></p>`;
                        feed += ']]></content:encoded>';
                    }
                }

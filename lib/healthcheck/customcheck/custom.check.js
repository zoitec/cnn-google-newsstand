                        ...
                        async.parallel({
                            entertainment: function (callback) {// GET LAST 10 PUBLISHED STORIES AND GET MOST RECENT STORY PUBLISHED OVER 10 MINUTE AGO
                               // return fetch('http://hypatia.api.cnn.com/svc/content/v2/search/collection1/type:article/section:us/dataSource:cnn/sort:lastModifiedDate/rows:10')
                                return fetch('http://hypatia.api.cnn.com/svc/content/v2/search/collection1/type:article/dataSource:cnn/sort:lastModifiedDate/rows:10')
                                .then((response) => {
                                    let message;
                                ...
                                .then((theDocs) => {
                                    ...
                                    let latestDocsOver10MinutesOld = [];

                                    if (theDocs) {
                                        theDocs.docs.forEach(function (model) {

                                            if (moment().diff(model.lastModifiedDate, 'minutes') > 10) {
                                                latestDocsOver10MinutesOld.push({doc: model, modifiedDate: model.lastModifiedDate});
                                            }

                                        });

                                        latestDocsOver10MinutesOld.sort(function(a,b){
                                          return new Date(b.modifiedDate) - new Date(a.modifiedDate);
                                        });
                                    }
                                    /* ################# TEST BLOCK ######################*/
                                    console.log('################### TEST BLOCK - GOING THROUGH FEED STORIES ########################');
                                    if (latestDocsOver10MinutesOld) {
                                        latestDocsOver10MinutesOld.forEach(function (model) {
                                            console.log('THIS STORY IS ***NOT***  OLDER THAN 10 MINS: ', model.doc.url, ' LAST MODIFIED DATE: ', model.doc.lastModifiedDate);
                                        });
                                    }
                                    console.log('most recent story over ten mins is: ', latestDocsOver10MinutesOld[0].doc.url, ' Modified at: ', latestDocsOver10MinutesOld[0].doc.lastModifiedDate);
                                    /* ################# TEST BLOCK ######################*/
                                    console.log('END################### TEST BLOCK - GOING THROUGH FEED STORIES ########################');

                                    return latestDocsOver10MinutesOld[0].doc;
                                })
                                ...
                                .then((feedAndStory) => {// CHECK TO SEE IF THE MOST RECENT STORY OVER TEN MINUTES OLD IS IN THE CURRENT FEED
                                    ...
                                    if (feedAndStory && feedAndStory.feed && feedAndStory.feed.items && feedAndStory.feed.items.length) {
                                        feedAndStory.feed.items.forEach( function (item) {
                                            console.log('FEED LINK: ', item.link[0].trim())
                                            if (item.link[0].trim() === feedAndStory.doc.url) {
                                                console.log('THIS story does Exist in the feed the story from HYPATIA');
                                                feedAsJSON.feedUpToDate = true;
                                            } else {
                                                console.log('THIS feed story is NOT the story from HYPATIA feed link is: ', item.link[0].trim(), '  HYPATIA link is: ',feedAndStory.doc.url);
                                            }
                                        });
                                    }
                                    console.log('END####END############### TEST BLOCK - GOING THROUGH FEED STORIES ########################');
                                    /* ################# TEST BLOCK ######################*/
